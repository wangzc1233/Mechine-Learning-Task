...
class FeatureBasedAttention(nn.Module):
    """基于特征的注意力机制，允许指定特定特征作为query来源"""
    def __init__(self, embed_dim, num_heads, dropout=0.0, query_feature_indices=None):
        ...
        self.query_feature_indices = query_feature_indices  # 指定作为query来源的特征索引
        
        # 如果指定了query特征索引，为query创建一个特殊的投影层
        if query_feature_indices is not None:
            self.q_proj = nn.Linear(len(query_feature_indices), embed_dim)
        else:
    ...
    def forward(self, x, attn_mask=None, key_padding_mask=None):
        ...
        # 如果指定了query特征索引，则只使用这些特征作为query来源
        if self.query_feature_indices is not None:
            # 提取指定特征作为query来源
            query_features = x[:, :, self.query_feature_indices]
            # 使用所有特征作为key和value来源
            key_features = value_features = x
        else:
            # 默认使用所有特征
            query_features = key_features = value_features = x
        
        # 投影query, key, value
        q = self.q_proj(query_features).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        k = self.k_proj(key_features).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        v = self.v_proj(value_features).reshape(batch_size, seq_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
...
# 新增：基于特征的Transformer编码器层
class FeatureBasedTransformerEncoderLayer(nn.Module):
    """基于特征的Transformer编码器层"""
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, 
                 activation="relu", norm_first=False, query_feature_indices=None):
        super(FeatureBasedTransformerEncoderLayer, self).__init__()
        
        # 基于特征的多头注意力
        self.self_attn = FeatureBasedAttention(
            embed_dim=d_model, 
            num_heads=nhead, 
            dropout=dropout,
            query_feature_indices=query_feature_indices
        )
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        if self.norm_first:
            ...
            # 先注意力后归一化
            x = self.norm1(x + self.dropout1(self.self_attn(x, attn_mask=src_mask, 
                                                          key_padding_mask=src_key_padding_mask)))
            # 先前馈后归一化
...
# 新增：基于特征的Transformer编码器
class FeatureBasedTransformerEncoder(nn.Module):
    """基于特征的Transformer编码器"""
    def __init__(self, encoder_layer, num_layers, norm=None):
        super(FeatureBasedTransformerEncoder, self).__init__()
...
# 修改：增强版Transformer预测模型（使用基于特征的注意力）
class EnhancedTransformerPredictor(nn.Module):
    """增强版Transformer预测模型（使用基于特征的注意力）"""
    def __init__(self, input_size, d_model, nhead, num_layers, output_size, dropout=0.2, query_feature_indices=None):
        ...
        # 基于特征的Transformer编码器层
        encoder_layer = FeatureBasedTransformerEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dropout=dropout,
        ...
        # 基于特征的Transformer编码器
        self.transformer_encoder = FeatureBasedTransformerEncoder(
...
class PowerPredictionSystem:
    ...
    def load_and_process_data(self):
        ...
        print(f"特征示例: {feature_columns}")
        #['Global_reactive_power', 'Voltage', 'Global_intensity', 'Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3', #0-5
        # 'RR', 'NBJRR1', 'NBJRR5', 'NBJRR10', 'NBJBROU', 'Sub_metering_remainder', #6-11
        # 'DayOfWeek', 'Month', 'DayOfYear', 'Weekend', 'Quarter', 'Season', 'Global_active_power_lag1', 'Global_active_power_lag7', 'Global_active_power_lag30'] #12-20
        ...
        self.feature_columns = feature_columns  # 保存特征列名称
    ...
    def train_model(self, seq_length=90, d_model=128, nhead=8, num_layers=3, 
                   epochs=150, batch_size=64, learning_rate=0.0001, query_feature_indices=None):
        """训练增强版Transformer模型（添加验证集和基于特征的注意力）"""
        print(f"\n开始训练Transformer模型 (序列长度: {seq_length})")
        print(f"模型参数: d_model={d_model}, nhead={nhead}, num_layers={num_layers}")
        
        if query_feature_indices is not None:
            print(f"使用特征索引 {query_feature_indices} 作为query来源")
            feature_names = [self.feature_columns[i] for i in query_feature_indices]
            print(f"对应特征名称: {feature_names}")
    ...
    def run_experiments(self, num_experiments=5, query_feature_indices=None):
        """运行多次实验（带模型集成）"""
        print("开始进行多次实验...")
...
def main():
    ...
    # 指定特征B和E作为query来源的索引
    # 假设特征B和E分别对应索引1和4（根据实际特征顺序调整）
    query_feature_indices = [6, 7,8,9,10]  # 这里需要根据实际特征索引调整
...
# 后台执行命令:
# nohup python /data/zcwang/temp/ml/transformer4.py > /data/zcwang/temp/logs/cross_transformer4.out 2>&1 &