...
class PowerPredictionSystem:
    ...
    def run_experiments(self, num_experiments=1):
        """运行多次实验（减少实验次数以节省时间）"""
        print("开始进行多次实验...")
...
def main():
    ...
    # 运行实验（减少实验次数）
    results, best_models, loss_histories = system.run_experiments(num_experiments=1)
...
# nohup  python /data/zcwang/temp/ml/Transformer.py  >/data/zcwang/temp/logs/transformer.out 2>&1 &